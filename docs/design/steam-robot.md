...
| 阶段 | 内容 | 产出 |
|---|---|---|
| **P0 验证 spike** | spawn Mob → 包成 `RebarEntity` → `tick()` → `pathfinder.moveTo()` → 重载恢复 | 证明地基跑通（最小） |
| **P1 本体+燃料+巡逻** | 部署物品、蒸汽朋克外观、SteamCanister 耗汽、巡逻、低汽回坞、坞 GUI | 能动、能充能的机器人 |
| **P2 采矿/砍树** | 工作区、寻路到目标方块、Pylon 正确破坏、回收掉落、破坏耗汽 | 第一种真实作业 |
| **P3 补料/搬运** | 存储 ↔ 产线入口/机器输入 的取送、路径点 | 物流作业 |
| **P4 深度联动+打磨** | Pylon 本体交互、多机协作、Display 外观层、（可选）可编程指令层 | 完善与扩展 |
