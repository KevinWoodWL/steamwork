...
| **P1b 充汽舱充能** | 充汽舱识别机器人并充其内置蒸汽（粒子/声音同玩家）、四方向绑定、低汽寻路回充 | 能自维持的机器人 | ⏳ 下一步 |
| **P1c 坞/控制 GUI** | 机器人 GUI（绑定充汽舱、查看燃料/状态、设工作区） | 可配置的机器人 | 待办 |
| **P2 采矿/砍树** | 工作区、寻路到目标方块、Pylon 正确破坏、回收掉落、破坏耗汽 | 第一种真实作业 | 待办 |
| **P3 补料/搬运** | 存储 ↔ 产线入口/机器输入 的取送、路径点 | 物流作业 | 待办 |
| **P4 深度联动+打磨** | Pylon 本体交互、多机协作、铜傀儡拿取/放下动画、（可选）可编程指令层 | 完善与扩展 | 待办 |

> 燃料表示：P1a 用机器人内置的蒸汽 double（容量来自 settings），非可插拔 `SteamCanister` 物品——
> 「像玩家一样充汽」指的是<b>充能体验</b>（粒子/声音），存储用内置值更简单。可插拔罐槽留作后期可选增强。
